...
bool RoomList::is_collide(std::_List_iterator<Room> room1, std::_List_iterator<Room>room2){
	...
	return fabs(coord1[0]-coord2[0]) <= (size1[0]+ size2[0]+1)*1.2 &&
		fabs(coord1[1]-coord2[1]) <= (size1[1]+ size2[1]+1)*1.2;
...
void RoomList::AddWalkRooms(){
	...
	for(Room room: list){
		...
		for(auto it = graph->a[i].begin(); it != graph->a[i].end();){

			k = (num1_y - num2_y)/(num1_x - num2_x);
			if( ((room.x_left-num1_x)*k + num1_y - room.y_bottom)* ((room.x_right-num1_x)*k + num1_y - room.y_bottom) < -1 && (room.x_left/2+room.x_right/2-num1_x)*(room.x_left/2+room.x_right/2-num2_x) < 0 ){
				if(flag){
				graph->a.push_back({});
				...
				graph->a[*it_del].remove(i);
				graph->a[i].erase(it_del);



				graph->addEdge((char)i, (char)(graph->a.size()-1));
				graph->addEdge(*it_del, (char)(graph->a.size()-1));
